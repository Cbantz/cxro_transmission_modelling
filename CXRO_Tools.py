...
def _post(data):
    url = urllib.request.urljoin(URLBASE, '/cgi-bin/' + script)
    return requests.post(url, data)

def _retrieve_data(response):
    soup = bs4.BeautifulSoup(response.text, 'html.parser')
    urldata = soup.body.h2.find('a').attrs['href']

    url = urllib.request.urljoin(URLBASE, urldata)
    with urllib.request.urlopen(url) as fp:
        return fp.read()
...
def _process(data):
    response = _post(data)
    s = _retrieve_data(response)
    return _parse_data(s)

def _create_data(chemical_formula : str, thickness : float, energy_min : int, energy_max : int, steps : int = 100):
    """Returns dictionary containing all necessary values for accessing CXRO Filter Transmission Data"""
    data = {}
    data['Materia'] = "Enter Formula"
    data['Formula'] = chemical_formula
...
def get_transmission(chemical_formula : str, thickness : float, energy_min : int, energy_max : int, steps : int = 100):
    """
    Returns ordered pairs of [energy (eV), transmission]. Takes arguments for options on CXRO Filter Transmission calculator.
    ...
    Returns:
        list: List of ordered pairs relating each energy in dataset to its transmission.
    """
    data = _create_data(chemical_formula, thickness, energy_min, energy_max, steps)
